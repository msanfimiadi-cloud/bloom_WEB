...
const LEGACY_WEB_API_BASE_URL = "https://bloomclub.ru/api/v1";

function isBrowserAppOrigin(): boolean {
  if (typeof window === "undefined") return false;
  return window.location.hostname === "app.bloomclub.ru";
}
...
function normalizeApiBaseUrl(rawBaseUrl: string | undefined): string {
  ...
  try {
    ...
    const legacyUrl = new URL(LEGACY_WEB_API_BASE_URL);
    const normalizedPath = parsedUrl.pathname.replace(
      /(?:\/api\/v1)+$/,
      "/api/v1",
    );
    parsedUrl.pathname = normalizedPath || legacyUrl.pathname;

    if (
      isBrowserAppOrigin() &&
      parsedUrl.origin === legacyUrl.origin &&
      parsedUrl.pathname.endsWith(legacyUrl.pathname)
    ) {
      return DEFAULT_API_BASE_URL;
    }
    ...
    if (
      ...
      return parsedUrl.pathname.replace(/\/+$/, "") || DEFAULT_API_BASE_URL;
    }
    ...
    if (
      ...
      parsedUrl.pathname = `${parsedUrl.pathname.replace(/\/+$/, "")}${legacyUrl.pathname}`;
    }
...
export const AUTH_STORAGE_KEY = "bloom_club_tma_auth";
...
export const LOGIN_CODE_PATH = "/api/v1/auth/login-code";
...
export interface LoginCodeResponse extends AuthResponse {
...
function shouldRetryCatalogError(error: CatalogLoadError): boolean {
  ...
  return (
    error.diagnostic.fetchPhase === "network_catch" &&
    error.diagnostic.abortSource !== "external"
  );
}
...
export function linkVkLoginCode(
  code: string,
): Promise<LinkingConfirmResponse> {
  return request<LinkingConfirmResponse>("/clients/me/linking/vk-login-code", {
    method: "POST",
