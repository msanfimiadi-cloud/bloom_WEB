...
const CONTENT_GET_RETRY_ATTEMPTS = 1;
...
async function contentRequestAttempt<T>(path: string, attempt: number): Promise<T> {
...
function shouldRetryContentRequest(error: unknown): boolean {
...
async function contentRequest<T>(path: string): Promise<T> {
  ...
  for (let attempt = 1; attempt <= CONTENT_GET_RETRY_ATTEMPTS + 1; attempt += 1) {
    try {
      return await contentRequestAttempt<T>(path, attempt);
    } catch (error) {
      lastError = error;

      if (attempt > CONTENT_GET_RETRY_ATTEMPTS || !shouldRetryContentRequest(error)) {
        throw error;
      }
    }
  }
