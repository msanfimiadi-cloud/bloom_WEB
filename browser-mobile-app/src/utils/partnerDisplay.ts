...
function isDevOrTestDiagnosticsEnabled(): boolean {
  return Boolean(
    import.meta.env.DEV ||
      import.meta.env.MODE === 'test' ||
      (typeof window !== 'undefined' && (window.localStorage.getItem('BLOOM_DEBUG') === '1' || window.sessionStorage.getItem('BLOOM_DEBUG') === '1')),
  );
}

export function tracePartnerImageDiagnostic(event: string, partner: Partner | null | undefined, normalizedImageUrl?: string, extra: Record<string, unknown> = {}): void {
  if (!isDevOrTestDiagnosticsEnabled()) {
    return;
  }

  const record = (partner && typeof partner === 'object' ? partner : {}) as Partner & Record<string, unknown>;
  console.info('partner_image_diagnostic', {
    event,
    partnerId: record.id ?? record.partner_id,
    ...
    selectedImageCandidate: extra.selectedImageCandidate ?? normalizedImageUrl,
    normalizedImageUrl,
    ...extra,
  });
}
...
export function normalizePartnerMediaUrlForDisplay(value: unknown): string | undefined {
...
function collectMediaValues(value: unknown): unknown[] {
  ...
  if (typeof value === 'object') {
    ...
    return [
      ...
      collectMediaValues(field(value, 'image')),
      collectMediaValues(field(value, 'photo')),
      collectMediaValues(field(value, 'file')),
    ];
  }
...
export function getPartnerImages(partner: Partner | null | undefined): string[] {
  ...
  const candidates = [
    // Match public WEB card priority: active gallery/photo first, then cover, then logo.
    partner.photos,
    partner.photo_url,
    partner.image_url,
    partner.gallery,
  ...
  return Array.from(
    new Set(candidates.flatMap(collectMediaValues).map(normalizePartnerMediaUrlForDisplay).filter((url): url is string => Boolean(url))),
  );
}
...
export function getOfferImages(offer: Offer | null | undefined): string[] {
  ...
  return Array.from(
    new Set([offer.image_url, offer.photo_url, offer.photos].flatMap(collectMediaValues).map(normalizePartnerMediaUrlForDisplay).filter((url): url is string => Boolean(url))),
  );
}
...
export function normalizeTelHref(phone: unknown): string | undefined {
  ...
  return digits.length >= 7 ? `tel:${normalized}` : undefined;
}
