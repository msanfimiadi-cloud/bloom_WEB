...
function getOfferAccentBadge(offer: Offer): string | undefined {
  ...
  if (prices.hasValidSaving) {
    return `Экономия ${formatMoney(prices.saving)}`;
  }

  const textCandidates = [discount, gift, value, benefit].filter(Boolean) as string[];
  return textCandidates.find((text) => !/%|скид/i.test(text) && /бонус|подар|выгод/i.test(text));
}
    ...
    <section className="page partner-page">
      ...
      <article className="partner-detail">
        ...
        {offersStatus === "loading" ? (
          <div className="offer-list" aria-busy="true" aria-label={offersLoadingTitle}>
            {Array.from({ length: 2 }).map((_, index) => (
              <article className="offer-card offer-card--skeleton" key={index}>
                <span className="offer-card__image skeleton-block" />
